...
and then moves all included manifests into a subfolder of the manifests folder. At the end
of the script, your original manifests folder remains untouched. If you like the results,
you can replace your old manifests folder with the new one this script generates.
'''
...
'''
Name of subfolder to put included manifests in. Cannot be blank or null for obvious
reasons (no point in running this script if you aren't putting included manifests in a
subfolder), but also because it will break the copy_modify_manifests function.
'''
...
def copy_modify_manifests(old_manifest, old_root, new_manifest_folder, subfolder, \
        is_included):
    print("Processing {}".format(old_manifest))
    ...
    try:
        ...
        print("Copy of {} failed".format(old_manifest))
        sys.exit(1)
    ...
    try:
        ...
        print("Unable to open {}".format(new_path))
        sys.exit(1)
    try:
        plist = plistlib.load(f)
    except:
        print("Unable to get contents of {}".format(new_path))
        sys.exit(1)

    # Modify the paths to the included manifests
    if 'included_manifests' in plist:
        old_includes = plist['included_manifests']
        # Even if the key itself exists, it may be an empty array, so check to see if
        # it's empty or not
        if old_includes:
    ...
    # Included manifests should never have catalogs, so delete those if needed
    if is_included == True and 'catalogs' in plist.keys():
        print("Removing catalogs from included manifest {}".format(old_manifest))
        del plist['catalogs']
    ...
    try:
        ...
        print("Could not write back plist changes to {}".format(old_manifest))
        sys.exit(1)
    f.close()
...
def main():
    ...
    if os.path.isdir(manifests):
        ...
        # The chances that a new directory with the exact timestamp of now already exists
        # is low, but we should double-check that directory doesn't already exist
        if not os.path.isdir(modified_manifests) and not os.path.isdir(new_subfolder):
            ...
            try:
                ...
                print("Unable to make directory {} for new manifest \
                    structure".format(modified_manifests))
                sys.exit(1)

            # Create a dictionary that stores any manifests that are not included in any
            # other manifests
            top_levels = {}
            ...
            for root, dirs, files in os.walk(manifests):
                for file in files:
                    ...
                    # If it's not in the included manifests list or the top level list,
                    ...
                    try:
                        ...
                        print("Unable to open {}".format(fullfile))
                        sys.exit(1)
                    try:
                        current_manifest = plistlib.load(f)
                    except:
                        print("Unable to get contents of {}".format(fullfile))
                        sys.exit(1)
                    f.close()
                    ...
                    if 'included_manifests' in current_manifest:
                        ...
                        # Even if the key itself exists, it may be an empty array, so
            ...
            # Now that we know which manifests are top-level manifests and which are
            # included ones, let's loop through both sets
            # Loop through the top-level manifests
            for top_level in top_levels:
                copy_modify_manifests(top_level, top_levels[top_level], \
                    modified_manifests, subfolder_name, False)
            ...
            for included_manifest in included_manifests:
                copy_modify_manifests(included_manifest, included_manifests[included_manifest], \
                    modified_manifests, subfolder_name, True)

        else:
            print("{} already exists as a directory.".format(modified_manifests))
            sys.exit(1)
    else:
        print("{} is not a valid manifests folder.".format(manifests))
        sys.exit(1)
